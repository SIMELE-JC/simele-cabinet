...
export default function Dashboard() {
  ...
  const createClientMutation = trpc.clients.create.useMutation({
    onSuccess: (newClient) => {
      ...
      if (newClient) navigate(`/clients/${newClient.id}/entretien-initial`);
    },
    ...
    <DashboardLayout>
      <div className="p-6 space-y-6">
        ...
        <div className="flex items-center justify-between flex-wrap gap-3">
          ...
          <div className="flex items-center gap-2">
            {/* Bouton accès rapide entretien initial sans client */}
            <Button variant="outline" className="gap-2" onClick={() => navigate("/entretien-initial")}>
              ...
              <div className="divide-y divide-border">
                {clients.map((client: Client) => (
                  ...
                  >
                    ...
                    {/* Bouton entretien initial rapide */}
                    <button
                      className="hidden sm:flex items-center gap-1 text-xs text-muted-foreground hover:text-primary transition-colors px-2 py-1 rounded hover:bg-primary/10"
                      onClick={(e) => { e.stopPropagation(); navigate(`/clients/${client.id}/entretien-initial`); }}
                      title="Démarrer un entretien initial"
                    >
